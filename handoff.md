...
- Статус: Windows 0.3.2 опубликован в `https://github.com/mikevisgit/KiloHub/releases/tag/v0.3.2` с VSIX и файлом SHA-256. Ветки `master` и `step4` опубликованы. Использован существующий проверенный артефакт без пересборки. Исторические записи ниже не заменяют актуальное evidence в `docs/step3-verification.md`. Ручная визуальная/NVDA-приёмка остаётся открытой.
- Ветка: `master`.
...
- Первый повторный `npm test` перед GitHub-публикацией: typecheck/lint PASS, unit-прогон остановлен внешним timeout 600000 ms во время масштабного suite. Второй `npm test` с timeout 1800000 ms завершился PASS: 112 unit, 39 component, minimum Extension Host 1.105.1; масштабный тест занял около 882 секунд. По уточнению пользователя дальнейшие необязательные прогоны прекращены: задача состоит в публикации уже работающего неизменённого выпуска, а не новом release gate. Предыдущий полный release gate 0.3.2 сохранён в `docs/step3-verification.md`.
- `gh release edit v0.3.2 --repo mikevisgit/KiloHub --draft=false --latest` опубликовал релиз. Target тега: production source `ce9fba0906e49f756b17b53bf01b1f94273d0ff8`. Размер и серверный digest загруженного VSIX совпали; файл дополнительно скачан через `gh release download` и его локальный `Get-FileHash -Algorithm SHA256` также совпал с исходным checksum.
- До прекращения дополнительных прогонов также завершились `npm run test:integration:current` (VS Code 1.138.0), `npm run verify:vsix` (13 entries) и `npm run test:verify-vsix-negative` (7/7): PASS. Повторные installed:min/current и audit не запускались, активных тестовых команд после остановки нет. GitHub API подтвердил `isDraft:false`, время публикации `2026-09-22T21:21:45Z`.
...
После проверки0.3.0 пользователь отклонил минимум трёх символов в каждом слове. Новый SEARCH-03: минимум для всего нормализованного запроса, короткие слова участвуют в AND/literal поиске; длинные токены отбирают папки первыми. Исправляются parser/предупреждение/поиск и регрессии, без migration/reimport. Ошибка появляется только по Enter. Внутренний error code short-token сохранён для совместимости protocol2, теперь означает короткий запрос. Повторные слова сохраняются в normalized query для идемпотентной повторной валидации. Установленный VSIX0.3.0 пока не изменён.

Выпуск0.3.0 установлен в основной VS Code и повторно установлен по просьбе пользователя после удаления расширения. CLI подтвердил `local.kilo-hub@0.3.0`. Артефакт `D:\VSCode\KiloHub\dist\kilo-hub-0.3.0-win32-x64.vsix`,54697bytes,SHA-256 `27EF318A3735E9D84CF107FE3329C29674D9A87EC9EBB4379827C89652F6038A`, source `b6bac3ff36bfdac2683f1bad62aa1b54ac4dc7dc`. Два clean/full/package цикла дали одинаковые байты:111unit/34component/audit0/minimum devhost PASS; current devhost и7negative PASS. Первый installed-min упал на cleanup repository-копии модуля; tests/integration/index.ts исправлен на activeExtension.extensionPath, тот же неизменённый VSIX прошёл повторные installed minimum/current с контролируемым PATH. Исходный npm run release не объявляется exit0. Независимый package review: PACKAGE APPROVED. Details asset/README проверены в архиве; реальная визуальная/NVDA приёмка и reload открытого окна остаются пользователю. Runtime-данные Kilo агентом не исследовались. Artifact/evidence commit ещё pending; Step4 не включён.

Финальный source review поиска/service/provider/UI/picker/metadata не нашёл подтверждённых Blocker/High/Medium (`reviews/step3-source-final-review.md`). Кандидат0.3.0 разрешён к source фиксации и упаковке, но package/installed approval этим не подменяется. Пользовательская визуальная приёмка остаётся отдельной.

22 сентября пользователь разрешил commit/build/install Step3. Финальное source review запущено; после него кандидат0.3.0 собирается из отдельного чистого Git checkout, чтобы не трогать незакоммиченные документы Step4. `.gitattributes` фиксирует LF для входящих в VSIX текстовых файлов и SVG, исключая checkout-зависимые README/icon hashes. До получения release evidence сборка и установка не объявляются выполненными.

Уточнения DB-10/SYNC-08 проверены: кеш integrity, durable FIFO/сравнение различий/partial search/restart, без обнуления готовой работы. Новая census-регрессия устранена bounded metadata chunks100/256KiB без изменения per-part staging;10001 case PASS642.63s (не ускорение к259s). Medium commit→initial pointer recovery закрыт и перепроверен. На финальном коде110 остальных unit PASS,34 component PASS, минимум/current Extension Host exit0 с TLS verification, typecheck/lint PASS. Полный npm test одним запуском не объявляется успешным: первая попытка имела900s timeout, крупный case проверен отдельно. Description Details/release notes актуализированы, README synthetic verifier positive+7negative PASS. Изменения ещё не закоммичены, VSIX0.3.0 не собирался/не устанавливался; следующий этап — финальное source/package review и выпуск. Подробности reviews/step3-sync-refinement-review.md.

Проверка уточнений 22 сентября: DB-10 7/7 на bundled minimum/current, включая replacement без предварительной ошибки open; renderer 17/17 с доступностью partial поиска. Integrated npm test остановлен timeout900s на10001-session case: регрессия новых per-row census commits, не прежнее допустимое время259s. Исправляются только scratch metadata transactions100rows/256KiB. Review дополнительно требует восстановление первой публикации после crash между session COMMIT и pointer write при плохом следующем диалоге. Ни общий gate, ни VSIX не считаются готовыми; см. `reviews/step3-sync-refinement-review.md`.

22 сентября пользователь согласовал DB-10/SYNC-08: начальные 259 s допустимы в фоне, запаздывание нормально, отдельная оптимизация мелких транзакций/SQL не нужна. quick_check только перед publication/первым открытием/после replacement или read error, без ослабления source guards. Новые и изменившиеся диалоги обновляются отдельно; полная сверка применяет различия. Прогресс при новых сообщениях сохраняется, готовая часть searchable с incomplete; более свежие версии идут следующим проходом. Реализация этих исправлений начата поверх незакоммиченного Gate C. VSIX не выпускался.

Gate B реализован и проверен: новые hubIndexEngine/Service/Worker/Protocol, hubSource/hubStorage; отдельные SQLite-поколения, durable progress, source read-only/DB-06/09/BLOB preflight, OS writer ownership, atomic pointer, last-good и автоматический фон без панели. Full npm test PASS: 83 unit/29 component/minimum Extension Host; 10 targeted cases на каждом bundled runtime, 10001 sessions, 12 MiB/NUL, worker death, настоящий SQLITE_FULL. Review 2 High/1 Medium закрыты повторной независимой проверкой. Неизвестный/нераспознанный coordination JSON fail-closed и не перезаписывается; восстановление проверено для распознанных SQLite-поколений. Production-scale corpus и installed Gate D ещё pending. API: HubIndexService start/subscribe/poll/stop, metadata-only snapshot; bundle build/hubIndexWorker.js. C может начинаться; версия пока 0.2.3, artifact не выпускался.

Gate B реализуется в рабочем дереве. Первое независимое review обнаружило 2 High/1 Medium восстановления last-good/инициализации/недоступного source parent; исправления выполняются до поиска (`reviews/step3-gate-b-review.md`). Тестовый runtime isolation review выявил и закрыл PATH casing/shim/cleanup дефекты; финальный Low порядка flags исправлен (`reviews/step3-runtime-isolation-review.md`). Current strict-PATH profile run прошёл; production release не выполнен.

Усилены проверки runtime: profile probe --without-node прошёл minimum/current с ENOENT для node/npm/sqlite3/kilo и рабочим встроенным FTS5. Это controlled PATH, не физическое удаление Node с машины. Worker pipe lifetime 4/4 на обоих runtime. Installed-smoke runner теперь до Production-mode активации передаёт явный отсутствующий synthetic KILO_DB под очищенным smokeRoot, поэтому startup не может обратиться к обычному источнику до создания fixture; PATH ограничен Windows, TLS verification включён. Запуск настоящего installed Step 3 пока ожидает готового VSIX.

Подготовлена независимая инфраструктура Details: `docs/extension-description.md` пока описывает только фактический baseline, package.mjs передаёт --readme-path, verifier проверяет canonical README/hash/Details asset. `step3-readme-package.mjs` подтвердил remap/bytes; `step3-readme-verifier.mjs` собрал synthetic VSIX и прошёл positive exact 12 entries + 7 negative cases. Независимое статическое review без findings. Это не production package/installed evidence; описание поиска и новые worker/version contracts добавляются после реализации B/C. Source discovery checkpoint зафиксирован `22fbbbd`.
