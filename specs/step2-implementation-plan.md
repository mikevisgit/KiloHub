...
### M10. Визуальный hotfix `0.2.1`

Зависимости: пользовательская проверка установленного `0.2.0`.

- [x] Зафиксировать отклонение `0.2.0`: browser default для `hidden` перекрыт production CSS, поэтому служебные подписи видны у всех карточек; стандартный Webview `body` padding складывается с внутренним gutter и создаёт чрезмерные поля.
- [x] Восстановить безусловный `[hidden]`, убрать внешний `body` padding, растянуть panel/list/card/detail/actions на доступную ширину и сохранить нормативный внутренний `--hub-row-x`.
- [x] Перенести визуальную рамку с оболочки панели на отдельные карточки и закрепить layout/visibility contract component-тестами.
- [ ] Повторить полный automated/review/package/install gate для версии `0.2.1`; evidence и артефакт `0.2.0` считать историческими, а не финальными.
- [ ] Получить пользовательский visual verdict на установленном `0.2.1`.

Gate M10: `0.2.1` проходит M6–M8 на одном source commit/hash, установлен в основной профиль и затем проходит повторную пользовательскую визуальную проверку.

### M11. Tooltip hotfix `0.2.2`

Этап M12 (`0.2.3`): единственный hover полного пути на inline `.folder-name`, отсутствие popup на focus и остальных элементах; package PNG вместо стандартной иконки страницы. Automated/source/package/install gate пройден: `62/62` unit, `29/29` component, независимые reviews без findings, две одинаковые сборки, exact verification с PNG и установка в основной профиль. Результаты и checksum в `handoff.md`; пользовательский visual verdict ожидается.
