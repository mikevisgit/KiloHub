...
Публикация на GitHub: существующая история опубликована в публичном `mikevisgit/KiloHub`, ветки `master` и `step4` подтверждены GitHub API. По явному решению пользователя три устаревших изображения из `req/step2/screenshots/` удалены только из актуального дерева; их присутствие в старых коммитах разрешено после пользовательской проверки. Исторические ссылки на эти baseline-изображения относятся к прежним коммитам. Материалы Step 4 опубликованы отдельно в ветке `step4`; production macOS не заявляется. GitHub Release `v0.3.2` опубликован с существующим проверенным VSIX без пересборки и файлом SHA-256. Повторно скачанный артефакт имеет тот же checksum; второй `npm test` PASS (112 unit/39 component/minimum host). Ручная визуальная/NVDA-приёмка остаётся открытой. Аудит публикации: `reviews/github-publication-audit.md`.
