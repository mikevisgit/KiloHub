...
for(const file of ['01-monograms.html','vscode-monograms/02-inset.html']){const html=fs.readFileSync(new URL(file,import.meta.url),'utf8');const names=[...html.matchAll(/class="name" data-tip="([^"]+)">([^<]+)<\/span>/g)];assert.equal(names.length,file.startsWith('01')?20:15);for(const [,path,name]of names)assert.equal(path,`D:\\Примеры\\${name}`.replaceAll('\\\\','\\'));assert(!html.includes('${'));assert(html.includes('interaction.cjs'));}
console.log('PASS regressions: paths35, strict/leap/timezone dates, midnight/current-first, latest accordion intent (normal/reduced), tooltip focus/pointer/Escape/timers/viewport bounds. DOM rendering not tested.');
