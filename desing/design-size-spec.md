...
Исключение по последующему запросу: original/default монограмм сохраняет прежние `--tone/--tone-ink` из refinements.css и border `--hub-outline`; новая цветовая логика эту тему не обрабатывает. Геометрия неизменна. Повторяющаяся прежняя палитра применяется к16примерам, остальные6панелей сохраняют16адаптивных групп.
