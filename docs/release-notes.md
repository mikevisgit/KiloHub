...
Kilo Hub показывает в Activity Bar локальные папки Windows, для которых в текущей локальной базе Kilo есть обычные root sessions без архива. Версия 0.2.0 заменяет нативное дерево утверждённым доступным Webview «Монограммы», не изменяя read-only источник и набор действий.
...
Версия распространяется как platform-specific пакет `kilo-hub-0.2.0-win32-x64.vsix`. Точный checksum и результаты release gate записываются в корневой `handoff.md`, который не включается в runtime-пакет.
