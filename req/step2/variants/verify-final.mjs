...
let panels=0;
for(const [file,count]of [['01-monograms.html',4]]){
...
assert.equal(walk(root).filter(f=>f.endsWith('.html')).length,2,'Only gallery and selected design');
const activeDesign=walk(path.resolve(root,'..')).filter(f=>!f.split(path.sep).includes('old_donotuse'));
for(const file of activeDesign.filter(f=>/\.(html|md)$/.test(f))){
 ...
 for(const ref of refs){if(/^https?:/.test(ref))continue;const [p,anchor]=ref.split('#'),dest=p?path.resolve(path.dirname(file),decodeURIComponent(p)):file;assert(fs.existsSync(dest),`${file}: ${ref}`);assert(!dest.split(path.sep).includes('old_donotuse'),'Active link points into archive');if(anchor&&dest.endsWith('.html'))assert(fs.readFileSync(dest,'utf8').includes(`id="${anchor}"`));}
}
...
console.log('PASS: 1 concept, 4 themes; structure, local dependencies, dates and ordering. No browser/runtime rendering asserted.');
...
for(const file of activeDesign.filter(f=>/\.(css|js|cjs|mjs)$/.test(f))){if(file===fileURLToPath(import.meta.url))continue;assert(!/vscode-monograms|\.inset[ .{]/.test(fs.readFileSync(file,'utf8')),'Obsolete design branch: '+file);}
