...
Нормативное приложение к [каноническому ТЗ](ТЗ%20реализации%20дизайна%20VSIX.md) с учётом более приоритетного [Уточнения ТЗ](02-уточнение-ТЗ.md), включая УТЗ-10. Описывает выбранный дизайн в4темах. Источники в порядке каскада: [demo-themes.css](variants/demo-themes.css), [panel.css](variants/panel.css), [refinements.css](variants/refinements.css), [scale.css](variants/scale.css). Локальные `interaction.cjs`/`demo.js` сохраняют прежний behavioral reference и не подтверждают tooltip-контракт УТЗ-10. Это анализ исходников и нормативные уточнения, не измеренный браузерный рендер.
...
По УТЗ-10 единственный visual owner и anchor полного пути: текст `.folder-name`, не вся `.folder-head` и не свободное место строки имени. У каждой папки собственный fullpath tooltip-узел со стабильными текстом/DOM ID и `role="tooltip"`; одновременно видим не более одного. Узел использует `position:fixed`, z-index20 и скрывается через `hidden`, `tabindex` не получает. Кнопка `.folder-head` постоянно ссылается на полный path через неизменяемый `aria-describedby`, созданный до focus; текст имени не становится отдельным Tab-stop. Popup имеет `pointer-events:none`, не содержит интерактивных потомков. Info/refresh/actions/head вне текста имени/date/mono/history/conversations/current/missing/стрелка не показывают popup по hover/focus; нативные `title`-подсказки запрещены.

Показ немедленный только по hover текста имени. Видимость: `!dismissed && sourceHovered`; focus сам по себе не показывает и не удерживает popup. Leave текста скрывает немедленно даже при focus на head, без grace. По УТЗ-09 попадание pointermove внутрь или на границу popup скрывает его в том же событии, включая owner под popup. Escape скрывает сразу. В обоих случаях dismissal удерживается до настоящего leave/reentry текста имени, независимо от focus. Правило одинаково для collapsed/expanded/current/missing. Полный path доступен через ARIA head без визуального popup по focus; полный conversation title постоянно присутствует в accessibility tree один раз.

Предпочтительная точка — `left=owner.left`, `top=owner.bottom` для текста `.folder-name`; если снизу места недостаточно, popup переносится над owner. Итоговые координаты clamp по X/Y с отступом8px внутри viewport Webview. `max-width:min(330px,calc(100vw - 16px))`, `max-height:none`, `width:max-content`, `overflow:visible`, `overflow-wrap:anywhere`. Scroll/resize/zoom/изменение текста запускают повторное измерение. Внутреннего непрокручиваемого обрезания нет; очень длинный текст может быть обрезан только физической границей Webview и остаётся полностью доступен через `aria-describedby` кнопки `.folder-head`.
