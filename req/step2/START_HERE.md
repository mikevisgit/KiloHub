...
Проверки макета — структурные/арифметические/офлайн JS. Браузерная визуальная и клавиатурная приёмка, реальная анимация/прокрутка/темы, Webview-интеграция и VSIX install/E2E остаются этапами реализации. Tooltip не может рисоваться за поверхностью Webview поверх соседнего редактора; принятые unclamped popup могут обрезаться. Эти ограничения не требуют выбора другого дизайна.
